...
export default {
  ...
  data() {
    return {
      ...
      busqueda: "",
      colores: {
        1: "#22c55e", 2: "#3b82f6", 3: "#a855f7", 4: "#f59e0b",
        5: "#ef4444", 6: "#14b8a6", 7: "#f97316", 8: "#0ea5e9",
        9: "#84cc16", 10: "#e11d48"
      },
      slots: [
  ...
  methods: {
    async cargarHorarios() {
      try {
        // Llamar al backend que obtiene HORARIOS + DOCENTES + MATERIAS + DIAS
        const datos = await window.electronAPI.invoke("getHorariosConDias");
        this.bloques = datos.flatMap((d, index) => {
          // Un horario puede tener varios días, crear un bloque por cada día
          return d.dias.map(dia => ({
            id: `${d.ID_HORARIO}-${dia}`,
            dia,
...
<style scoped>
/* Reutiliza tus estilos existentes de VistaDeHorario */
